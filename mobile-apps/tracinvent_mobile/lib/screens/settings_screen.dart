...
class SettingsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer<SettingsProvider>(
      builder: (context, settingsProvider, _) {
        ...
        final pagePadding = isCompact ? 14.0 : 32.0;
        return Scaffold(
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 24),
                      _buildSyncIntegrationSettings(context),
                      const SizedBox(height: 24),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Manage app preferences and sync',
                  maxLines: 1,
  ...
  Widget _buildSyncIntegrationSettings(BuildContext context) {
    final isCompact = MediaQuery.sizeOf(context).width < 700;
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              const Row(
                children: [
                  Icon(Icons.sync_alt, color: Color(0xFF64748B), size: 20),
                  SizedBox(width: 8),
                  Text(
              ...
              Text(
                'Connect mobile app to desktop API and verify both sides have matching data after sync.',
                style: TextStyle(
                  fontSize: 13,
                  color: Colors.grey.shade600,
                  height: 1.4,
                ...
                child: Row(
                  children: [
                    ...
                    FilledButton.tonalIcon(
                      onPressed: () => _showSyncServerConfigDialog(context),
                ...
                children: [
                          ...
                          : () async {
                              ...
                              if (context.mounted) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text(
                                      syncProvider.syncStatus == SyncStatus.success
                                          ? 'Sync completed successfully.'
                                          : 'Sync failed: ${syncProvider.errorMessage ?? 'Unknown error'}',
                                    ),
                                    backgroundColor:
                                        syncProvider.syncStatus == SyncStatus.success
                  ...
                  SizedBox(
                    ...
                    child: OutlinedButton.icon(
                      onPressed: () => _compareSyncedData(context),
  ...
  Future<void> _showSyncServerConfigDialog(BuildContext context) async {
    final syncProvider = context.read<SyncProvider>();
    final controller = TextEditingController(text: syncProvider.serverBaseUrl);

    final saved = await showDialog<bool>(
      context: context,
      builder: (context) {
        return AlertDialog(
          title: const Text('Desktop API Server URL'),
          content: TextField(
    ...
    if (saved == true) {
      await syncProvider.updateServerBaseUrl(controller.text);
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
  ...
  Future<void> _compareSyncedData(BuildContext context) async {
    final syncProvider = context.read<SyncProvider>();
    try {
      if (context.mounted) {
        showDialog(
          ...
          builder: (_) => const AlertDialog(
            content: Row(
              children: [
                CircularProgressIndicator(),
                SizedBox(width: 14),
                Expanded(child: Text('Comparing local vs desktop data...')),
              ],
            ),
          ),
        );
      ...
      final comparison = await syncProvider.compareWithServer();
      if (context.mounted) {
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          title: Text(comparison.isMatch ? 'Data Matched' : 'Data Difference Found'),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _buildCompareLine('Products', comparison.localProducts, comparison.serverProducts),
              const SizedBox(height: 6),
              _buildCompareLine('Stock Rows', comparison.localStockRows, comparison.serverStockRows),
              const SizedBox(height: 6),
              _buildCompareLine(
                'Transactions',
                comparison.localTransactions,
                comparison.serverTransactions,
              ),
              const SizedBox(height: 12),
              Text(
                comparison.isMatch
                    ? 'Mobile and desktop data counts are aligned.'
                    : 'Run Sync Now and compare again. If mismatch remains, check pending operations.',
                style: TextStyle(
                  fontSize: 12,
  ...
  Widget _buildCompareLine(String label, int local, int server) {
    final match = local == server;
    return Row(
      children: [
        ...
        Text(
          ...
          style: TextStyle(
            fontWeight: FontWeight.w600,
            color: match ? const Color(0xFF10B981) : Colors.red,
          ),
        ),
      ],
    );
