...
class _TransactionsScreenState extends State<TransactionsScreen> {
                ...
                child: const Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 12),
                      Text('Loading data...', style: TextStyle(fontWeight: FontWeight.w500)),
                    ],
                  ),
                ),
              ),
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      leading: Container(
                        ...
                        child: Icon(
                          isStockIn ? Icons.add_shopping_cart : Icons.shopping_cart_checkout,
                          color: isStockIn ? const Color(0xFF10B981) : const Color(0xFF3B82F6),
                          size: 24,
                        ),
                      ),
                      title: Text(
  ...
  void _showAdjustmentDialog(BuildContext context) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Stock adjustment feature coming soon!'),
  ...
  void _showTransactionDialog(BuildContext context, String type) {
    ...
    final inventoryProvider = Provider.of<InventoryProvider>(context, listen: false);
    final warehouseProvider = Provider.of<WarehouseProvider>(context, listen: false);
    final stockEntryProvider = Provider.of<StockEntryProvider>(context, listen: false);

    if (inventoryProvider.items.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please add inventory items first in Inventory screen')),
      );
      return;
    }
    ...
    if (warehouseProvider.warehouses.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please add warehouses first in Warehouses screen')),
      );
      return;
    }
    
    debugPrint('Opening transaction dialog - Items: ${inventoryProvider.items.length}, Warehouses: ${warehouseProvider.warehouses.length}');
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) {
          ...
          void filterItems(String query) {
            ...
            setState(() {
              filteredItems = inventoryProvider.items.where((item) {
          ...
          return AlertDialog(
            title: Text(type == 'purchase' ? 'Record Purchase (Stock In)' : 'Record Sale (Stock Out)'),
                          ...
                          child: DropdownButtonFormField<String?>(
                            ...
                            items: [
                              ...
                              ...cells.map((cell) {
                                return DropdownMenuItem(
                                  value: cell.id,
                                  child: Text('${cell.name} (${cell.code})', overflow: TextOverflow.ellipsis),
                                );
                              }),
                            ],
                            onChanged: (value) => setState(() => selectedCellId = value),
                          ),
                        ),
                      ],
                    ),
                                    ...
                                    Material(
                                      ...
                                      child: InkWell(
                                        onTap: () {
                                          ...
                                          if (current > 1) {
                                            quantityController.text = (current - 1).toStringAsFixed(0);
                                            setState(() {});
                                          }
                                        },
                      ...
                      children: [
                        ...
                        Expanded(
                          child: TextField(
                            ...
                            decoration: InputDecoration(
                              labelText: type == 'purchase' ? 'Supplier' : 'Customer',
                              border: const OutlineInputBorder(),
                            ),
                          ),
                        ),
            ...
            actions: [
              ...
              ElevatedButton(
                ...
                onPressed: () async {
                  ...
                  if (quantityController.text.isEmpty || unitPriceController.text.isEmpty) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Please fill quantity and price')),
                    );
                    return;
                  }
                  ...
                  final transaction = Transaction(
                    ...
                    supplier: type == 'purchase' && partyController.text.isNotEmpty ? partyController.text : null,
                    customer: type == 'sale' && partyController.text.isNotEmpty ? partyController.text : null,
                    notes: notesController.text.isEmpty ? null : notesController.text,
                    transactionDate: selectedDate,
                    createdAt: DateTime.now(),
                  );

                  await inventoryProvider.addTransaction(transaction, type == 'purchase');

                  if (context.mounted) {
                    Navigator.pop(context);
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('${type == 'purchase' ? 'Purchase' : 'Sale'} recorded successfully'),
                        backgroundColor: type == 'purchase' 
                            ? const Color(0xFF10B981)
                            : const Color(0xFF3B82F6),
                      ),
                ...
                child: Text('Record ${type == 'purchase' ? 'Purchase' : 'Sale'}'),
              ),
            ],
          );
        },
